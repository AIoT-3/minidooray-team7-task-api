...
class TaskUpdateRequestTest {
    ...
    @Test
    void testValidTaskUpdateRequest() {
        TaskUpdateRequest request = new TaskUpdateRequest("Valid Name", "Valid Content");
    ...
    void testNameIsBlank() {
        TaskUpdateRequest request = new TaskUpdateRequest(" ", "Valid Content");
        Set<ConstraintViolation<TaskUpdateRequest>> violations = validator.validate(request);
    ...
    void testNameIsTooLong() {
        ...
        TaskUpdateRequest request = new TaskUpdateRequest(longName, "Valid Content");
        Set<ConstraintViolation<TaskUpdateRequest>> violations = validator.validate(request);
        assertThat(violations).hasSize(1);
    ...
    void testContentIsBlank() {
        TaskUpdateRequest request = new TaskUpdateRequest("Valid Name", " ");
        Set<ConstraintViolation<TaskUpdateRequest>> violations = validator.validate(request);
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getMessage()).isEqualTo("must not be blank");
    }
    ...
    void testContentIsTooLong() {
        ...
        TaskUpdateRequest request = new TaskUpdateRequest("Valid Name", longContent);
        Set<ConstraintViolation<TaskUpdateRequest>> violations = validator.validate(request);
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getMessage()).contains("length must be between 0 and 200");
    }
